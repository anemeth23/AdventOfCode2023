...
function getNum(str) {
  ...
  for (let k = 0; k < str.length; k++) {
    for (let i = 0; i < num.length; i++) {
      str[k] === num[i] && resultArray.push(num[i]);
    }
  }
  return Number(resultArray[0] + resultArray[resultArray.length - 1]);
}

let sum = 0;
for (const x of adventFirstDay) {
  sum += getNum(x);
}
console.log(sum);
...
// this Map will be used for deciphering the strings that actually mean numbers and to collect simple number values
const numMapOrig = new Map([
  ["one", 1],
  ["two", 2],
  ["three", 3],
  ["four", 4],
...
function iterateThroughString(str) {
  // collect every extracted value into a Map
  const numMap = new Map([]);

  // iterate over the string type numbers and store them with their indexes in the numMap Map
  for (const [numStr] of numMapOrig) {
    if (str.indexOf(numStr) !== -1) numMap.set(str.indexOf(numStr), numStr);
  }

  // do the same for the last occurence of every string type numbers and store them with their indexes in the numMap Map
  for (const [numStr] of numMapOrig) {
    if (str.lastIndexOf(numStr) !== -1)
      numMap.set(str.lastIndexOf(numStr), numStr);
  }

  // iterate through the strings looking for the number type numbers and store them with indexes in the numMap Map
  for (let k = 0; k < str.length; k++) {
    for (let i = 0; i < num.length; i++) {
      str[k] === num[i] && numMap.set(k, Number(num[i]));
    }
  }

  // initialize two variables to select the first and last numbers; they will be used to create the desired two digit numbers
  let firstNum;
  let secondNum;

  // define the first digit of the number; if the value with the lowest index in the numMap Map is a string use the numMapOrig to get the corresponding number, e.g. 'eight' => 8
  if (typeof numMap.get(Math.min(...numMap.keys())) === "string") {
    firstNum = numMapOrig.get(numMap.get(Math.min(...numMap.keys())));
    // if it is not a string, than just retrieve the number itself
  } else {
...
// initialize a variable to calc the sum
let sumPart2 = 0;
// iterate through the array and invite the above function on every string
for (const x of adventFirstDay) {
